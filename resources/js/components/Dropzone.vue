<template>
  <input type="file" accept="image/jpeg" ref="fileInput" class="hidden" @input="uploadFile" />
  <div class="cursor-pointer m-auto text-center" v-if="$page.props.uploadedImage">
    <span class="rounded-lg overflow-hidden shadow-2xl block" v-html="$page.props.uploadedImage" @click="$refs.fileInput.click()"></span>
    <button class="bg-purple-800 hover:bg-purple-700 text-white font-bold py-2 px-4 rounded inline-flex items-center my-5 mx-auto" @click="downloadHTML">
  ...
  <div v-else
    class="border-2 border-dashed border-purple-800 h-[200px] w-[200px] rounded-lg flex justify-center items-center cursor-pointer m-auto" @click="$refs.fileInput.click()"
  >
    <svg
      version="1.1"
      class="h-8 text-grey mr-2"
    ...
    <span class="block text-grey text-purple-800 font-bold">Try it!</span>
  </div>
</template>
...
export default {
  setup() {
    ...
    function downloadHTML() {
      window.open(`/download?path=${usePage().props.value.path}`,'_blank');
    }

    return { fileInput, uploadFile, downloadHTML }
  }
};
</script>
